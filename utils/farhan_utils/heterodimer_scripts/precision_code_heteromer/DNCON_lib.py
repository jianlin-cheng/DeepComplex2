...
def build_dataset_dictionaries_file(path_lists, list_name):
  length_dict = {}
  with open(path_lists + 'L.txt') as f:
    for line in f:
      cols = line.strip().split()
      length_dict[cols[0]] = int(cols[1])
  tr_l = {}
  print ("Length of length_dict=",len(length_dict))
  with open(path_lists + list_name) as f:
    for line in f:
      if (line.strip().split("_")[0] not in length_dict) or (line.strip().split("_")[1] not in length_dict):
          print ("Damn!!!!!!!! "+line)
          continue
      else:
        tr_l[line.strip()] = length_dict[line.strip().split("_")[0]]+length_dict[line.strip().split("_")[1]]
  # print ('Data counts:')
  # print ('Total : ' + str(len(length_dict)))
  # print ('Train : ' + str(len(tr_l)))
  return (tr_l)
  ...
  with open(path_lists + 'train.lst') as f:
    for line in f:
      if (line.strip().split("_")[0] not in length_dict) or (line.strip().split("_")[1] not in length_dict):
        ...
        tr_l[line.strip()] = length_dict[line.strip().split("_")[0]]+length_dict[line.strip().split("_")[1]]
        tr_l_tup[line.strip()]=(length_dict[line.strip().split("_")[0]],length_dict[line.strip().split("_")[1]])
  return (tr_l,tr_l_tup)
  ...
  with open(path_lists + 'test.lst') as f:
    for line in f:
      if (line.strip().split("_")[0] not in length_dict) or (line.strip().split("_")[1] not in length_dict):
        ...
        te_l[line.strip()] = length_dict[line.strip().split("_")[0]]+length_dict[line.strip().split("_")[1]]
        te_l_tup[line.strip()] = (length_dict[line.strip().split("_")[0]],length_dict[line.strip().split("_")[1]])
  return (te_l,te_l_tup)
  ...
  with open(path_lists + 'sample.lst') as f:
    for line in f:
      if (line.strip().split("_")[0] not in length_dict) or (line.strip().split("_")[1] not in length_dict):
        ...
        ex_l[line.strip()] = length_dict[line.strip().split("_")[0]]+length_dict[line.strip().split("_")[1]]
  return (ex_l)
...
def load_sample_data_2D(data_list, path_of_X, path_of_Y,seq_end, min_seq_sep,dist_string, reject_fea_file='None'):#seq_end = count
  import pickle
  data_all_dict = dict()
  print("######### Loading data\n")
  accept_list = []
  ...
  #The following is used to extract the number of features from a sample
  ex_l = build_dataset_dictionaries_sample(data_list)
  sample_dict = subset_pdb_dict(ex_l, 30, 500, seq_end, 'random') #count=seq_end #can be random ordered
  ...
  for i in range(0,len(sample_name)):
    ...
    featurefile =path_of_X + '/' + 'feat-'  + pdb_name + '.txt'
  ...
  return feature_num
##################################################################################################################################################
def load_sample_data_2D_intra(data_list, path_of_X, path_of_Y, seq_end, min_seq_sep,dist_string, reject_fea_file='None', sep=True):#seq_end = count
  import pickle
  data_all_dict = dict()
  print("######### Loading data\n")
  accept_list = []
  ...
  for i in range(0,len(sample_name)):
    ...
    if "feat_stage2" in path_of_X or "feat_stage_2" in path_of_X:
        featurefile=path_of_X+'/'+pdb_name+'.feat.stage2.txt'
    else:
        featurefile =path_of_X + '/' + 'feat-'  + pdb_name + '.txt'
    if ((len(accept_list) == 1 and ('# cov' not in accept_list and '# plm' not in accept_list and '# plmc' not in accept_list and '# pre' not in accept_list and '# spa' not in accept_list and '# netout' not in accept_list)) or 
          (len(accept_list) == 2 and ('# cov' not in accept_list or '# plm' not in accept_list or '# plmc' not in accept_list or '# pre' not in accept_list or '# spa' not in accept_list or '# netout' not in accept_list)) or (len(accept_list) > 2)):
    ...
    if '# netout' in accept_list:      
      if not os.path.isfile(netout):
                  print("netout matrix file not exists: ",netout, " pass!")
                  continue  
    intra_file=path_of_X+"/intra/"+pdb_name+".dncon2.rr"
    if '# intra' in accept_list:      
      if not os.path.isfile(intra_file):
                  print("Intrachain file does not exists: ",intra_file, " pass!")
                  continue  

    ### load the data
    (featuredata,feature_index_all_dict) = getX_2D_format_intra_test(featurefile, cov, plm, plmc, pre, spa, netout, intra_file, accept_list, pdb_lens, notxt_flag,sep=sep)
    # print("\n######",len(featuredata))

    feature_num = len(featuredata)
  return feature_num #, featuredata, feature_index_all_dict
...
########################################################################################################################################3
def get_y_from_this_list(selected_ids, path, min_seq_sep, l_max, y_dist, len_A, len_B, lable_type = 'bin'):
  ...
  for pdb in selected_ids:
    ...
    break
  if lable_type == 'bin':
    y_file = path + y_dist + sample_pdb + '.txt'
    if not os.path.isfile(y_file):
      print("%s not exits!" % y_file)
      return False
    y = getY_eval(y_file, min_seq_sep, l_max, lA=len_A, lB=len_B)
  elif lable_type == 'real':
    y_file = path + sample_pdb + '.txt'
    if not os.path.isfile(y_file):
      print("%s not exits!" % y_file)
      return False
    y = getY_eval(y_file, 0, l_max)
  if (len_A * len_B != len(y)):
    print ('Error!! y does not have L * L feature values!!')
    return False
  Y = np.zeros((xcount, len_A * len_B))
  i = 0
  if lable_type == 'bin':
    for pdb in sorted(selected_ids):
      Y[i, :]  = getY_eval(path + pdb + '.txt', min_seq_sep, l_max, len_A, len_B)
      i = i + 1
  elif lable_type == 'real':
    for pdb in sorted(selected_ids):
      Y[i, :]  = getY_eval(path + pdb + '.txt', 0, l_max)
      i = i + 1
  print ("list Y.shape=",Y.shape)
  return Y
#############################################################################################################################################
#Previous version
"""
def get_y_from_this_list(selected_ids, path, min_seq_sep, l_max, y_dist, len_A, len_B, lable_type = 'bin'):
  ...
  if lable_type == 'bin':
    ...
    if not os.path.isfile(y_file):
      print("%s not exits!" % y_file)
      return False
    y = getY(y_file, min_seq_sep, l_max, lA=len_A, lB=len_B)
  elif lable_type == 'real':
    y_file = path + sample_pdb + '.txt'
    if not os.path.isfile(y_file):
      print("%s not exits!" % y_file)
  ...
  if lable_type == 'bin':
    for pdb in sorted(selected_ids):
      Y[i, :]  = getY(path + pdb + '.txt', min_seq_sep, l_max, len_A, len_B)
      i = i + 1
  elif lable_type == 'real':
    for pdb in sorted(selected_ids):
...
############################################################################################################################################
#get binary y lable
def getY(true_file, min_seq_sep, l_max, lA, lB):
  # calcualte the length of the protein (the first feature)
  L = 0
  true_Y=np.loadtxt(true_file)
  len_A=true_Y.shape[0]
  len_B=true_Y.shape[1]
  if lA != len_A or lB != len_B:
    return np.zeros((1))
  """
  ...
  i = 0
  if len_A > l_max and len_B > l_max:
    return true_Y[0:l_max,0:l_max]
  if len_A > l_max:
    return true_Y[0:l_max,0:len_B]
  if len_B > l_max:
    return true_Y[0:len_A,0:l_max]

  Y[0:len_A, 0:len_B] = true_Y
  ...
  return Y
########################################################################################################################################3
#used during evaluation for getting labels as shape Y (len_A, leb_B, 0)
def getY_eval(true_file, min_seq_sep, l_max, lA, lB):
  # calcualte the length of the protein (the first feature)
  L = 0
  true_Y=np.loadtxt(true_file)
  len_A=true_Y.shape[0]
    ...
    with open(feature_file) as f:
      ...
      for line in f:
        ...
        if len(this_line) == 1:
          # 0D feature
          #continue
          # to ignore scalars, remark everything below
          feature_namenew = feature_name + ' 0D'
          feature_index +=1
          if feature_index in feature_index_all_dict:
            print("Duplicate feature name ",feature_namenew, " in file ",feature_file)
...
def getX_2D_format_intra(feature_file, cov, plm, plmc, pre, spa, netout, intra_file, accept_list, pdb_len = 0, notxt_flag = True, logfile = None, sep=True):
  # calcualte the length of the protein (the first feature)
  if logfile != None:
    chkdirs(logfile)

  L = 0
  #print ("getX_2D_format_intra: feature_file: ",feature_file)
  Data = []
  feature_all_dict = dict()
  feature_index_all_dict = dict() # to make sure the feature are same ordered 
  feature_name='None'
  ...
  if '# intra' in accept_list: #2D feature
      #Write intra chain loading code here using readRR.py
      from readRR import readRRFile
      fsta,contents=readRRFile(intra_file)
      if len(fsta.strip())!=L:
          print ("Bad intrachain file. File should be in CASP RR format")
          sys.exit()
      cmap_intra=rr2npcmap(contents,L,sep)
      feature2D = cmap_intra #np.asarray(this_line).reshape(L, L)
      feature_index +=1
      feature_namenew = "# intrachain contacts" + ' 2D'
      if feature_index in feature_index_all_dict:
        print("Duplicate feature name ",feature_namenew, " in file ",intra_file)
        exit
      else:
        feature_index_all_dict[feature_index] = feature_namenew
...
#############################################################################################################################
def getX_2D_format_intra_test(feature_file, cov, plm, plmc, pre, spa, netout, intra_file, accept_list, pdb_len = 0, notxt_flag = True, logfile = None, sep=True):
  # calcualte the length of the protein (the first feature)
  if logfile != None:
    chkdirs(logfile)
...
def rr2npcmap(contents, L, sep=True):
    #print ("L=",L)
    #cmap=np.zeros((L,L))
    cmap=np.ones((L,L))
...
#########################################################################################################################################3
def get_x_2D_from_this_list(selected_ids, feature_dir, l_max,dist_string, len_A, len_B, reject_fea_file='None', pdb_len = 0,stage2=False,sep=True):
  xcount = len(selected_ids)
  sample_pdb = ''
  for pdb in selected_ids: # we are actually selecting pdb one at a time.
    sample_pdb = pdb
    break
  accept_list = []
  notxt_flag = True
  ...
  print ("get_x_2D_from_this_list: feature_dir: ",feature_dir)
  if ('feat_stage2' in feature_dir or "feat_stage_2" in path_of_X):
      featurefile =feature_dir + '/' + sample_pdb + '.feat.stage2.txt'
  else:
      print ("Using DNCON2 features from: ",feature_dir)
      featurefile = feature_dir + '/' + 'feat-'  + sample_pdb + '.txt'
  #if not stage2: featurefile =feature_dir + '/' + 'feat-'  + sample_pdb + '.txt'
  #if stage2: featurefile =feature_dir + '/' + sample_pdb + '.feat.stage2.txt' #/data/commons/DeepComplex_common/datasets/DNCON2_features_homodimers/feat_stage2/8PRK.feat.stage2.txt
  intra_file=feature_dir+"/intra/"+sample_pdb+".dncon2.rr"
  ...
  if ((len(accept_list) == 1 and ('# cov' not in accept_list and '# plm' not in accept_list and '# plmc' not in accept_list and '# pre' not in accept_list and '# spa' not in accept_list and '# netout' not in accept_list)) or 
    ...
    # print
    if not os.path.isfile(featurefile):
                print("feature file not exists: ",featurefile, " pass!","$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$ Here $$$$$$$$$$$$$$$$$$$$$$$$$$ ",feature_dir)   
                return False
  if '# cov' in accept_list:
    if not os.path.isfile(cov):
                print("Cov Matrix file not exists: ",cov, " pass!")
  ...
  if '# intra' in accept_list:      
    if not os.path.isfile(intra_file):
                print("Intrachain contact file does not exist: ",intra_file, " pass!")
                return False

  #(featuredata,feature_index_all_dict) = getX_2D_format(featurefile, cov, plm, plmc, pre, spa, netout, accept_list, pdb_len, notxt_flag)  
  (featuredata,feature_index_all_dict) = getX_2D_format_intra(featurefile, cov, plm, plmc, pre, spa, netout, intra_file, accept_list, pdb_len, notxt_flag,sep=sep)  
  ...
  for key in sorted(feature_index_all_dict.keys()):
      ...
      if feature.shape[0] == feature.shape[1]: #check if lengths are same
        feature_2D_all.append(feature)
      else:
...
#  X_2D = np.zeros((xcount, l_max, l_max, F_2D))
  X_2D = np.zeros((xcount, len_A, len_B, F_2D))
  ###################################################################
  #Up to this is all done to prepare the X_2D dimensions
  #Now we actually get the data in the correct order from the entire list
  ###################################################################
  pdb_indx = 0
  ...
  for pdb_name in sorted(selected_ids):
      ...
      if '# netout' in accept_list:      
        if not os.path.isfile(netout):
                    print("netout matrix file not exists: ",netout, " pass!")
                    continue 
      intra_file = feature_dir + '/intra/' + pdb_name + '.dncon2.rr'
      if '# intra' in accept_list:      
        if not os.path.isfile(intra_file):
                    print("Intrachain file does not exists: ",intra_file, " pass!")
                    continue 
...
#      X_tmp = np.zeros((L, L, F))
      X_tmp = np.zeros((len_A, len_B, F))
      for i in range (0, F):
        X_tmp[:,:, i] = feature_2D_all[i][0:len_A,len_A:]      
  ...
  return X_2D
#################################################################################################################################################
### Used during evaluation since training shape is (n,l_max,l_max,56)
def get_x_2D_from_this_list_eval(selected_ids, feature_dir, l_max,dist_string, len_A, len_B, reject_fea_file='None', pdb_len = 0,stage2=False,sep=True):
  xcount = len(selected_ids)
  sample_pdb = ''
  for pdb in selected_ids: # we are actually selecting pdb one at a time.
    sample_pdb = pdb
...
#      X_tmp = np.zeros((L, L, F))
    ...
    """
      ...
      X_2D[pdb_indx, :, :, :] = feature_2D_all_complete
      pdb_indx = pdb_indx + 1
      print ("Evaluation X_2D.shape=",X_2D.shape)
  return X_2D
...
def evaluate_prediction_4 (dict_l, P, Y, min_seq_sep):
  ...
  P3L10 = ceil_top_xL_to_one_4(dict_l, P2, Y, 0.1)
  P3L20 = ceil_top_xL_to_one_4(dict_l, P2, Y, 0.05)
  avg_prec_l5, avg_prec_l2, avg_prec_1l, avg_mcc_l5, avg_mcc_l2, avg_mcc_1l, avg_recall_l5, avg_recall_l2, avg_recall_1l, avg_f1_l5, avg_f1_l2, avg_f1_1l = print_detailed_evaluations_4(dict_l, P3L5, P3L10, P3L20, P2, Y)
...
def evaluate_prediction_one (dict_l, P, Y, min_seq_sep):
  P2 = floor_lower_left_to_zero(P, min_seq_sep)
  #datacount = len(Y[:, 0])
  #L = int(math.sqrt(len(Y[0, :])))
  #Y1 = floor_lower_left_to_zero(Y, min_seq_sep)
  ...
  return (avg_prec_l5, avg_prec_l2, avg_prec_1l, avg_mcc_l5, avg_mcc_l2, avg_mcc_1l, avg_recall_l5, avg_recall_l2, avg_recall_1l, avg_f1_l5, avg_f1_l2, avg_f1_1l)
#####################################################################################################################################################
def evaluate_prediction_5 (dict_l, P, Y, len_A, len_B, min_seq_sep):
  Y=Y.reshape(Y.shape[0],len_A*len_B) #Newly added
  P2 = floor_lower_left_to_zero(P, len_A, len_B, min_seq_sep)
  #datacount = len(Y[:, 0])
  #L = int(math.sqrt(len(Y[0, :])))
  #Y1 = floor_lower_left_to_zero(Y, min_seq_sep)
  ...
  P3L5 = ceil_top_xL_to_one_4(dict_l, P2, Y, 0.2, len_A, len_B)
  P3L10 = ceil_top_xL_to_one_4(dict_l, P2, Y, 0.1, len_A, len_B)
  P3L20 = ceil_top_xL_to_one_4(dict_l, P2, Y, 0.05, len_A, len_B)
  P3L30 = ceil_top_xL_to_one_4(dict_l, P2, Y, 1/30, len_A, len_B)
  P3T5 = ceil_top_xL_to_one_4(dict_l, P2, Y, 5, len_A, len_B)
  P3T10 = ceil_top_xL_to_one_4(dict_l, P2, Y, 10, len_A, len_B)
  avg_prec_l5, avg_prec_l10, avg_prec_l20, avg_prec_l30, avg_prec_T5, avg_prec_T10, avg_mcc_l5, avg_mcc_l10, avg_mcc_l20, avg_mcc_l30, avg_mcc_T5, avg_mcc_T10, avg_recall_l5, avg_recall_l10, avg_recall_l20, avg_recall_l30, avg_recall_T5, avg_recall_T10, avg_f1_l5, avg_f1_l10, avg_f1_l20, avg_f1_l30, avg_f1_T5, avg_f1_T10 = print_detailed_evaluations_5(dict_l, P3L5, P3L10, P3L20, P3L30, P3T5, P3T10, P2, Y)
  #return (avg_prec_l5, avg_prec_l2, avg_prec_1l, avg_mcc_l5, avg_mcc_l2, avg_mcc_1l, avg_recall_l5, avg_recall_l2, avg_recall_1l, avg_f1_l5, avg_f1_l2, avg_f1_1l)
  return (avg_prec_l5, avg_prec_l10, avg_prec_l20, avg_prec_l30, avg_prec_T5, avg_prec_T10, avg_mcc_l5, avg_mcc_l10, avg_mcc_l20, avg_mcc_l30, avg_mcc_T5, avg_mcc_T10, avg_recall_l5, avg_recall_l10, avg_recall_l20, avg_recall_l30, avg_recall_T5, avg_recall_T10, avg_f1_l5, avg_f1_l10, avg_f1_l20, avg_f1_l30, avg_f1_T5, avg_f1_T10)
#####################################################################################################################################################
...
def floor_lower_left_to_zero(XP, len_A, len_B, min_seq_sep=0):
  X = np.copy(XP)
  datacount = len(X[:, 0])
  L = int(math.sqrt(len(X[0, :])))
  X_reshaped = X.reshape(datacount, len_A, len_B)
  """
  for p in range(0,L):
    for q in range(0,L):
      if ( q - p < min_seq_sep):
        X_reshaped[:, p, q] = 0
  """
  X = X_reshaped.reshape(datacount, len_A * len_B)
  return X
...
def ceil_top_xL_to_one_4(ref_file_dict, XP, Y, x, len_A, len_B):
  X_ceiled = np.copy(XP)
  i = -1
  for pdb in sorted(ref_file_dict):
    i = i + 1
    if x<1:
        xL = int(x * (len_A+len_B))
    else:
        xL=x
    print ("xL=",xL)
    X_ceiled[i, :] = np.zeros(len(XP[i, :]))
    X_ceiled[i, np.argpartition(XP[i, :], -xL)[-xL:]] = 1
  print ("X_ceiled.shape=",X_ceiled.shape)
  return X_ceiled
...
def print_detailed_evaluations(dict_l, dict_n, dict_e, PL5, PL2, PL, Y):
  ...
  avg_pc_l2  = 0 # average predicted correct L/10
  avg_pc_1l  = 0 # average predicted correct L/20
  avg_acc_l5 = 0.0
  avg_acc_l2 = 0.0
  avg_acc_1l = 0.0
  list_acc_l5 = []
...
###############################################################################################################################################
def print_detailed_evaluations_5(dict_l, PL5, PL10, PL20, PL30, PT5, PT10, P, Y):
  datacount = len(dict_l)
  avg_prec_l5 = 0.0
  avg_prec_l10 = 0.0
  avg_prec_l20 = 0.0
  ...
  i = -1
  for pdb in sorted(dict_l):
    print (Y.shape,"vs",PL5.shape," ",PL10.shape)
    mcc_l5    = matthews_corrcoef(Y[i], PL5[i, :])
    mcc_l10    = matthews_corrcoef(Y[i], PL10[i, :])
    mcc_l20    = matthews_corrcoef(Y[i], PL20[i, :])
    mcc_l30    = matthews_corrcoef(Y[i], PL30[i, :])
    mcc_T5    = matthews_corrcoef(Y[i], PT5[i, :])
    mcc_T10    = matthews_corrcoef(Y[i], PT10[i, :])
    prec_l5   = precision_score(Y[i], PL5[i, :])
    prec_l10   = precision_score(Y[i], PL10[i, :])
    prec_l20   = precision_score(Y[i], PL20[i, :])
    prec_l30   = precision_score(Y[i], PL30[i, :])
    prec_T5   = precision_score(Y[i], PT5[i, :])
    prec_T10   = precision_score(Y[i], PT10[i, :])
    recall_l5 = recall_score(Y[i], PL5[i, :])
    recall_l10 = recall_score(Y[i], PL10[i, :])
    recall_l20 = recall_score(Y[i], PL20[i, :])
    recall_l30 = recall_score(Y[i], PL30[i, :])
    recall_T5 = recall_score(Y[i], PT5[i, :])
    recall_T10 = recall_score(Y[i], PT10[i, :])
    F1_l5     = f1_score(Y[i], PL5[i, :])
    F1_l10     = f1_score(Y[i], PL10[i, :])
    F1_l20     = f1_score(Y[i], PL20[i, :])
    F1_l30     = f1_score(Y[i], PL30[i, :])
    F1_T5     = f1_score(Y[i], PT5[i, :])
    F1_T10     = f1_score(Y[i], PT10[i, :])
    # pc_l5 = np.logical_and(Y[i], PL5[i, :]).sum()
    # pc_l2 = np.logical_and(Y[i], PL2[i, :]).sum()
    # pc_1l = np.logical_and(Y[i], PL[i, :]).sum()
    ...
    avg_mcc_T10 += mcc_T10
    avg_prec_l5 += prec_l5
    avg_prec_l10 += prec_l10
    avg_prec_l20 += prec_l20
    avg_prec_l30 += prec_l30
    avg_prec_T5 += prec_T5
    avg_prec_T10 += prec_T10
    avg_recall_l5 += recall_l5
    avg_recall_l10 += recall_l10
    avg_recall_l20 += recall_l20
    avg_recall_l30 += recall_l30
    avg_recall_T5 += recall_T5
    avg_recall_T10 += recall_T10
    avg_f1_l5 += F1_l5
    avg_f1_l10 += F1_l10
    avg_f1_l20 += F1_l20
    avg_f1_l30 += F1_l30
  ...
  avg_mcc_l10 /= datacount
  avg_mcc_l20 /= datacount
  avg_mcc_l30 /= datacount
  avg_mcc_T5 /= datacount
  avg_mcc_T10 /= datacount
  avg_prec_l5 /= datacount
  avg_prec_l10 /= datacount
  avg_prec_l20 /= datacount
  avg_prec_l30 /= datacount
  avg_prec_T5 /= datacount
  avg_prec_T10 /= datacount
  avg_recall_l5 /= datacount
  avg_recall_l10 /= datacount
  avg_recall_l20 /= datacount
  avg_recall_l30 /= datacount
  avg_recall_T5 /= datacount
  avg_recall_T10 /= datacount
  avg_f1_l5 /= datacount
  avg_f1_l10 /= datacount
  avg_f1_l20 /= datacount
  avg_f1_l30 /= datacount
  avg_f1_T5 /= datacount
  avg_f1_T10 /= datacount
  # print("   Avg                           %6s        %6s %6s %6s    %.4f    %.4f    %.4f" % (avg_nc, avg_pc_l5, avg_pc_l2, avg_pc_1l, avg_prec_l5, avg_prec_l2, avg_prec_1l))
  # print ("")
  return (avg_prec_l5, avg_prec_l10, avg_prec_l20, avg_prec_l30, avg_prec_T5, avg_prec_T10, avg_mcc_l5, avg_mcc_l10, avg_mcc_l20, avg_mcc_l30, avg_mcc_T5, avg_mcc_T10, avg_recall_l5, avg_recall_l10, avg_recall_l20, avg_recall_l30, avg_recall_T5, avg_recall_T10, avg_f1_l5, avg_f1_l10, avg_f1_l20, avg_f1_l30, avg_f1_T5, avg_f1_T10)

###############################################################################################################################################    
def print_detailed_evaluations_4(dict_l, PL5, PL10, PL20, P, Y):
  datacount = len(dict_l)
  ...
  for pdb in sorted(dict_l):
    mcc_l5    = matthews_corrcoef(Y[i], PL5[i, :])
    mcc_l2    = matthews_corrcoef(Y[i], PL10[i, :])
    mcc_1l    = matthews_corrcoef(Y[i], PL20[i, :])
    prec_l5   = precision_score(Y[i], PL5[i, :])
    prec_l2   = precision_score(Y[i], PL10[i, :])
    prec_1l   = precision_score(Y[i], PL20[i, :])
    recall_l5 = recall_score(Y[i], PL5[i, :])
    recall_l2 = recall_score(Y[i], PL10[i, :])
    recall_1l = recall_score(Y[i], PL20[i, :])
    F1_l5     = f1_score(Y[i], PL5[i, :])
    F1_l2     = f1_score(Y[i], PL10[i, :])
    F1_1l     = f1_score(Y[i], PL20[i, :])
    # pc_l5 = np.logical_and(Y[i], PL5[i, :]).sum()
    # pc_l2 = np.logical_and(Y[i], PL2[i, :]).sum()
    # pc_1l = np.logical_and(Y[i], PL[i, :]).sum()
    # prec_l5 = float(pc_l5) / (float(L5) + epsilon)
