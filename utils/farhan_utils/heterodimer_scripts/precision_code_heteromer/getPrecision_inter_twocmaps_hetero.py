...
#    print ("i=",i,"j=",j)
    ...
    for x in list(reversed(range(v+1))):
        if (j+x<l_B):
            jj_max=j+x
            break
#    print ("ii_min=",ii_min,"ii_max=",ii_max)
#    print ("jj_min=",jj_min,"jj_max=",jj_max)
    for x in range(ii_min,ii_max+1,1):
        for y in range(jj_min,jj_max+1,1):
...
def findContactInter(rr1,rr2,fasta,relax=0): #usually cmap1 is the pdb_rr, cmap2 is the predicted rr (dncon2)
    ...
    if (relax==0):
        ...
        #return
        L=len(fasta[0])+len(fasta[1])
        L_A=len(fasta[0])
        L_B=len(fasta[1])
        cmap1=np.zeros((L_A,L_B)) #pdb (true interchain)
        cmap2=np.zeros((L_A,L_B)) #predicted rr (dncon2)
        for rr1_item in rr1:
...
def getName(name):
    split=name.split("/")
    name=split[-1]
    name=name.split("-")[-1].replace(".txt","").replace(".rr","").replace("_.rr","")
    name=name
    return name
...
if (len(sys.argv)!=3): 
    ...
    name=getName(rr_file1) #Get from true file
    outfile_inter=makeOutputFile(rr_file1,"_inter_all.rr")
    outfile_pred=makeOutputFile(rr_file2,"_inter_pred_2L.rr")
    
    label1,fastaA1,fastaB1,rr1=readRRFile(rr_file1) #multimer PDB interchain rr
    label2,fastaA2,fastaB2,rr2=readRRFile(rr_file2) #prediction rr(dncon2.rr) (.dncon2_sorted.rr) already sorted before
    ...
    if (len(fastaA1)!=len(fastaA2) or len(fastaB1)!=len(fastaB2)): 
...
############################################################
    ...
    for relax_prec in range(relaxation):
        ...
        top["Top-5"]=str(round(getPrecisionInter(rr1,rr2,[fastaA2,fastaB2],"5",relax_prec),2))
        #print(contact_list)
        #contact_list=[]
        top["Top-10"]=str(round(getPrecisionInter(rr1,rr2,[fastaA2,fastaB2],"10",relax_prec),2))
        #print(contact_list)
        #contact_list=[]
        top["Top-L/10"]=str(round(getPrecisionInter(rr1,rr2,[fastaA2,fastaB2],"L/10",relax_prec),2))
        top["Top-L/5"]=str(round(getPrecisionInter(rr1,rr2,[fastaA2,fastaB2],"L/5",relax_prec),2))
        #print(contact_list)
        #contact_list=[]
        top["Top-L/2"]=str(round(getPrecisionInter(rr1,rr2,[fastaA2,fastaB2],"L/2",relax_prec),2))
        top["Top-L"]=str(round(getPrecisionInter(rr1,rr2,[fastaA2,fastaB2],"L",relax_prec),2))
        top["Top-2L"]=str(round(getPrecisionInter(rr1,rr2,[fastaA2,fastaB2],"2L",relax_prec),2))
