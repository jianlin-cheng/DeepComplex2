#usage: Rscript compare_rr_three.R <interchain.rr> <predictedintrachain.dncon2.rr> <true_intrachain_from_pdb.rr> <output_image_file>
args=commandArgs(trailingOnly = TRUE)
infile=args[1] # interchain RR file
infile2 =args[2] # intrachain (DNCON2) RR file
pdbfile=args[3] #pdbfile RR
outfile=args[4] #ouput image file
...
#Final output:
#Interchain_color: Red
#True_Intrachain_color: Blue
#Predicted_Intrachain_color: Greed
...
native<-data
plot(native$V1, native$V2, col="red", pch=0, xlab = NULL, ylab = NULL, ylim=c(2*L, 0), xlim=c(0, L*2))
points(native$V2, native$V1, col="red", pch=0, xlab = NULL, ylab = NULL, ylim=c(2*L, 0), xlim=c(0, L*2))
...
pdb <- subset(data3, data3$V4 == "8")
points(pdb$V1, pdb$V2, col="blue", xlab = NULL, ylab = NULL, ylim=c(2*L, 0), xlim=c(0, L*2), pch = 1)
