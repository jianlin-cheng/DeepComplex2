...
#Somehow preinput the fasta_dictionary here!
fasta_dict=readFastaDict("rr_fasta_dict.txt")
rr_file=os.path.abspath(sys.argv[1])
map_file=os.path.abspath(sys.argv[2])
seq_aln_file=os.path.abspath(sys.argv[3])
outfolder=os.path.abspath(sys.argv[4])+"/"
name_key=os.path.basename(rr_file).split(".")[0].split("_")[0]
if len(sys.argv)==6: name_key=sys.argv[5]
print ("Name_key=",name_key)
if not os.path.isdir(outfolder): os.makedirs(outfolder)
fasta,rr=readRRFile(rr_file)
key_list,seq_aln_dict=readSeqAlnFile(seq_aln_file)
if name_key in key_list:
    if (fasta==seq_aln_dict[name_key]):
        print("The fasta sequence in file matches that after alignment. Skipping reindexing!")
        os.system("scp "+rr_file+" "+outfolder)
        sys.exit()
    ...
    sys.exit("Key "+name_key+" not found in "+seq_aln_file+" Quitting!!!")
...
print(map_dict)
reindexed_rr=reindex_rr(rr,map_dict)
outfile=outfolder+os.path.basename(rr_file)
write2File(outfile,seq_aln_dict[name_key],reindexed_rr)
