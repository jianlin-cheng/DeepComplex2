...
usage="Usage: python "+sys.argv[0]+" <binding_file> <fasta_dictionary>"
...
for pair in fasta_pair_list:
    first=pair.strip().split()[0] #os.path.splitext(pair.strip().split(",")[0])[0]
    second=pair.strip().split()[1] #os.path.splitext(pair.strip().split(",")[1])[0]
